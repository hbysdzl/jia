...
<div class="page-heading">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="<?php echo U('userlis');?>">返回</a>
                </li>
...
<form id="form_data">    
<table  style="width:400px;text-align:center;height:400px;margin-left:20px;font-size:16px">
                
   <tr>
       <td>角色分配:</td>
       <td>
		  <?php if(is_array($role)): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['role_id']==$ad['role_id']):?>
	      <input type="radio" name="role_id" value="<?php echo ($vo["role_id"]); ?>" checked="checked" /><span><?php echo ($vo["role_name"]); ?></span>&nbsp&nbsp&nbsp
	      <?php else:?>
	      <input type="radio" name="role_id" value="<?php echo ($vo["role_id"]); ?>"/><span><?php echo ($vo["role_name"]); ?></span>&nbsp&nbsp&nbsp
	      <?php endif; endforeach; endif; else: echo "" ;endif; ?>
	   </td>
   </tr>
    <tr>
       ...
       <td>账号:</td>
       <td><input type="text" name="username" value="<?php echo ($ad["username"]); ?>" size="35"/></td>
   	</tr>
    <tr>
       <td>密码:</td>
   	...
   	<tr>
       <td>确认密码:</td>
       <td><input type="password" name="passwords" value="" size="35"/></td>
   	</tr> 
   	<tr>
       <td>是否启用:</td>
	   <td>
	     <?php if($ad['is_use']==1):?>
			<input type="radio" name="is_use" value="1" checked="checked"/>启用&nbsp &nbsp &nbsp &nbsp 
			<input type="radio" name="is_use" value="0"  />禁用
	     <?php else:?>
			<input type="radio" name="is_use" value="1" />启用&nbsp &nbsp &nbsp &nbsp 
	        <input type="radio" name="is_use" value="0" checked="checked" />禁用
	     <?php endif;?>
	   </td>
       ...
       <td>
			<?php if($ad['is_edit']==1):?>
			<input type="radio" name="is_edit" value="1" checked="checked"/>可修改&nbsp &nbsp &nbsp &nbsp 
			<input type="radio" name="is_edit" value="0"  />不可修改
	     <?php else:?>
			<input type="radio" name="is_edit" value="1" />可修改&nbsp &nbsp &nbsp &nbsp 
	        <input type="radio" name="is_edit" value="0" checked="checked" />不可修改
	     <?php endif;?>
	   </td>
       ...
       <td>
			<?php if($ad['is_del']==1):?>
			<input type="radio" name="is_del" value="1" checked="checked"/>可删除&nbsp &nbsp &nbsp &nbsp 
			<input type="radio" name="is_del" value="0"  />不可删除
			<?php else:?>
			<input type="radio" name="is_del" value="1" />可删除&nbsp &nbsp &nbsp &nbsp 
	        <input type="radio" name="is_del" value="0" checked="checked" />不可删除
			<?php endif;?>
	   </td>
		...
		success:function(data){
			if(data.ok==0){
				...
				$('#tishi').fadeOut(3000);
					...
					setTimeout(function(){
						location.href="<?php echo U('userlis');?>"
					},3000);
