﻿(function(){
		...
		if(comments[sceneName].maxPage == null){
			if(!comments[sceneName].initing){
				...
				tempFun = function(){
					...
					$.ajax({
						type:"get",
						url:serverDomain+"/index.php?s=/Member/Panoapi/readComment",
						timeout: 10000,
						data: params,
						//dataType:'json',
						dataType:'jsonp',
	...
	function sendComment(){
		...
		$.ajax({
			type:"get",
			url:serverDomain+"/index.php?s=/Member/Panoapi/saveComment",
			timeout: 5000,
			data: params,
			dataType:'jsonp',
			jsonp: "saveComment",
	...
	function init(){
		...
		$.ajax({
			type:"get",
			url:serverDomain+"/index.php?s=/Member/Panoapi/commentInit",
			timeout: 5000,
			data: params,
			dataType:'jsonp',
			jsonp: "commentInit",
	...
	function headReplace(){
		...
		$("div").each(function(i,e){
			if(e.style.backgroundImage.indexOf("hotspotComment/avatar.png?headimg") != -1){
				...
				e.parentNode.children[1].children[0].children[0].style.border = "1px solid rgba(0, 0, 0, 0)";   //控制头像圆角
				flag1 = true;
				return;
			}
		});
		...
		$("div").each(function(i,e){
			if(e.style.backgroundImage.indexOf("hotspotComment/line-icon.png?zindex") != -1){
			    e.parentNode.style.zIndex = "3000";
				flag3 = true;
				return;
			}
		});
		if(!(flag1 && flag2 && flag3)) setTimeout(headReplace, 500);
		headState = false;
	}
