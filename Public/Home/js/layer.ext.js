/**
 ...
 @Desc: 本拓展会持续更新
 @editDate: 2016.06.17
 @editForm: Feng_zhenjie@126.com

 **/
 
layer.use('../css/skin/layer.ext.css', function(){
    layer.ext && layer.ext();
});
...
layer.photos = function(options){
    ...
    if(json){
        ...
        var imgs = $(page.parent).eq(page.startparent).find('img');
		var nowimg = imgs.eq(page.start);
...
layer.photosPage = function(options){
    var log = {};
    log.run = function(index, startparent){
        layer.photos({
            html: options.html,
    ...
    options = options || {};
	$(options.parent).each(function(index) {
        // var http = new XMLHTMLRequest();
        // var url = "";
