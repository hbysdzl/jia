...
;!function (win, $, undefined) {
	var SVGMap = (function () {
		...
		SVGMap.prototype.options = {
			...
			// 地区颜色
			pathColor: ['fff', 'fdead1', 'f7b553', 'ffea71'],
			stateDataType: 'json',
		...
		SVGMap.prototype.render = function () {
			...
			// 显示位置图标
			for(var i in coordinatefig.icons){
				var icons = coordinatefig['icons'][i];
				var iconsNa = icons.split(",");

				$(".experWrap a").append('<div class="addIcon addIcon' + i +'"></div>');
				$(".addIcon"+i).css({
					'left': iconsNa[0] + 'px',
					'top': iconsNa[1] + 'px'
				});
			...
			for (var i in mapConfig.shapes) {
				var thisStateData = stateData[i],
					initColor = '#' + mapConfig['colors'][i],
				...
				stateColor[i].initColor = initColor;
				stateColor[i].hoverColor = hoverColor;
				stateColor[i].selectedColor = selectedColor;

				var obj = r.path(mapConfig['shapes'][i]);
				obj.id = i;
				obj.name = mapConfig['names'][i];
				obj.fill = mapConfig['colors'][i];
				obj.address = mapConfig['address'][i];
				obj.attr(attributes);

				// 镇区名显示
				var textList = mapConfig['adds'][i];
				var textNa = textList.split(",");
				r.text(textNa[0],textNa[1],textNa[2]);

				if (opt.external) {
					opt.external[obj.id] = obj;
				}
				     
				var piece = obj || $(".addIcon") || textList;

				if (stateData[i] && stateData[i].diabled) {
					obj.attr({
						fill: disabledColor,
						cursor: 'default'
					});
					...
					// 判断只有已开业或装修中的镇区才显示提示框
					if (mapConfig['nums'][i][0] == 3) {
						// $(".addIcon").hover(function(e){
